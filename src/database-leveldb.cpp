...
bool Database_LevelDB::dbSaveBlock(const v3s16 &pos, const std::string &data)
{
	leveldb::Status status = m_database->Put(leveldb::WriteOptions(),
			i64tos(getBlockAsInteger(pos)), data);
	if (!status.ok()) {
		warningstream << "dbSaveBlock: LevelDB error saving block "
...
void Database_LevelDB::dbLoadBlock(const v3s16 &pos, std::string *block)
{
	std::string datastr;
...
bool Database_LevelDB::dbDeleteBlock(const v3s16 &pos)
{
	leveldb::Status status = m_database->Delete(leveldb::WriteOptions(),
			i64tos(getBlockAsInteger(pos)));
	if (!status.ok()) {
		warningstream << "dbDeleteBlock: LevelDB error deleting block "
			<< PP(pos) << ": " << status.ToString() << std::endl;
		return false;
	}
