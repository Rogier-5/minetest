...
public:
	...
	{
		bool deleted = dbDeleteBlock(pos);
		if (deleted)
			m_blocks_not_found.insert(pos);
		return deleted;
	}
...
protected:
	virtual bool dbSaveBlock(const v3s16 &pos, const std::string &data) = 0;
	virtual void dbLoadBlock(const v3s16 &pos, std::string *block) = 0;
	virtual bool dbDeleteBlock(const v3s16 &pos) = 0;
private:
