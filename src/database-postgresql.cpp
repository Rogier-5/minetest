...
bool Database_PostgreSQL::dbSaveBlock(const v3s16 &pos,
		const std::string &data)
{
	// Verify if we don't overflow the platform integer with the mapblock size
	if (data.size() > INT_MAX) {
		errorstream << "Database_PostgreSQL::dbSaveBlock: Data truncation! "
...
void Database_PostgreSQL::dbLoadBlock(const v3s16 &pos,
		std::string *block)
{
	verifyDatabase();
...
bool Database_PostgreSQL::dbDeleteBlock(const v3s16 &pos)
{
	verifyDatabase();
