...
bool Database_SQLite3::dbDeleteBlock(const v3s16 &pos)
{
	verifyDatabase();
	...
	if (!good) {
		warningstream << "dbDeleteBlock: Block failed to delete "
			<< PP(pos) << ": " << sqlite3_errmsg(m_database) << std::endl;
	}
	return good;
}

bool Database_SQLite3::dbSaveBlock(const v3s16 &pos, const std::string &data)
{
	verifyDatabase();
...
void Database_SQLite3::dbLoadBlock(const v3s16 &pos, std::string *block)
{
	verifyDatabase();
