...
bool Database_Redis::dbSaveBlock(const v3s16 &pos, const std::string &data)
{
	...
	if (!reply) {
		warningstream << "dbSaveBlock: redis command 'HSET' failed on "
	...
	if (reply->type == REDIS_REPLY_ERROR) {
		warningstream << "dbSaveBlock: saving block " << PP(pos)
...
void Database_Redis::dbLoadBlock(const v3s16 &pos, std::string *block)
{
	std::string tmp = i64tos(getBlockAsInteger(pos));
	...
	case REDIS_REPLY_ERROR: {
		...
		freeReplyObject(reply);
		errorstream << "dbLoadBlock: loading block " << PP(pos)
...
bool Database_Redis::dbDeleteBlock(const v3s16 &pos)
{
	std::string tmp = i64tos(getBlockAsInteger(pos));
	...
	if (!reply) {
		...
		warningstream << "dbDeleteBlock: deleting block " << PP(pos)
			<< " failed: " << std::string(reply->str, reply->len) << std::endl;
		freeReplyObject(reply);
		return false;
	}
